package services

import (
	"github.com/aronipurwanto/go-api-gorm/models"
	"github.com/aronipurwanto/go-api-gorm/repositories"
)

type EmployeeService interface {
	GetAll() ([]models.Employee, error)
	GetAllPaginated(page, limit int) ([]models.Employee, int64, error)
	GetByID(id int) (*models.Employee, error)
	Create(emp *models.Employee) error
	Update(emp *models.Employee) error
	Delete(id int) error
}

type employeeServiceImpl struct {
	repo repositories.EmployeeRepository
}

func NewEmployeeService(repo repositories.EmployeeRepository) EmployeeService {
	return &employeeServiceImpl{repo}
}

func (s *employeeServiceImpl) GetAll() ([]models.Employee, error) {
	return s.repo.GetAll()
}

func (s *employeeServiceImpl) GetAllPaginated(page, limit int) ([]models.Employee, int64, error) {
	return s.repo.GetPaginated(page, limit)
}

func (s *employeeServiceImpl) GetByID(id int) (*models.Employee, error) {
	return s.repo.GetByID(id)
}

func (s *employeeServiceImpl) Create(emp *models.Employee) error {
	return s.repo.Create(emp)
}

func (s *employeeServiceImpl) Update(emp *models.Employee) error {
	return s.repo.Update(emp)
}

func (s *employeeServiceImpl) Delete(id int) error {
	return s.repo.Delete(id)
}
